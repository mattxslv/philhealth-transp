...
export const metadata: Metadata = {
  ...
  description: "PhilHealth financial statements, revenue, expenses, and fund utilization reports.",
};
...
export default function FinancialsPage() {
  ...
  const loadData = () => {
    ...
    Promise.all([
      ...
      axios.get("/data/financials-2007.json").catch(() => ({ data: null }))
    ])
      .then(([financialsRes, detailed2007Res]) => {
        ...
        // Merge detailed 2007 data into the annualReports array
        ...
        setDetailedData(detailed2007Res.data);
        setLoading(false);
      })
      .catch(err => {
  ...
  const revenueData = {
    labels: latestData.breakdown?.revenueBySource 
      ? latestData.breakdown.revenueBySource.map((item: any) => item.source)
      : ["Direct Contributors", "Indirect Contributors"],
    datasets: [{
      data: latestData.breakdown?.revenueBySource
        ? latestData.breakdown.revenueBySource.map((item: any) => item.amount / 1000000000)
        : [
            (latestData.breakdown?.directContributors?.revenue || 0) / 1000000000,
            (latestData.breakdown?.indirectContributors?.revenue || 0) / 1000000000
          ],
